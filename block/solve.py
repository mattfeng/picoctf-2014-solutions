...
flag = open('./encrypted', 'rb').read()

# meet in the middle attack

keyspace = int('FFFFFF', 16) + 1

decrypted_candidates = {}

# ostream = open('./decrypted_once', 'wb')

# for key2 in xrange(0, keyspace):
#     if key2 % 100000 == 0:
#         print key2
#     reverse = decrypt(flag, key2)[:18]

#     ostream.write(reverse + '*' * 10)

#     decrypted_candidates[reverse] = key2

# print '[i] finished creating key2 candidates'

decrypted = open('./decrypted_once', 'rb').read().split('*' * 10)
for i, d in enumerate(decrypted):
    decrypted_candidates[d.encode('hex')[:18]] = i

del decrypted

print '[i] loaded decrypted once'

for key1 in xrange(0, keyspace):
    forward = encrypt('message: ', key1).encode('hex')
    if key1 % 100000 == 0:
        print key1
    try:
        works = decrypted_candidates[forward]
        print 'key2:', works
        print 'key1:', key1
    except:
        pass
