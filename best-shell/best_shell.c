...
typedef struct input_handler {
    char cmd[32]; // overflow cmd into handler, call handler after
    void (*handler)(char *);
} input_handler;
...
void rename_handler(char *arg){
    ...
    if (found != NULL){
        strcpy(found->cmd, new); // no bounds checking
    }else{
...
void auth_admin_handler(char *arg){
    ...
    if (!strcmp(arg, admin_password)){
        admin = true; // 0x80489a3 address of instruction
...
void shell_handler(char *arg){
    if (admin){
        gid_t gid = getegid();
        setresgid(gid, gid, gid); // instr address: 0x80489f8 - just jump here
        system("/bin/sh");
    }else{
